...
from agents.utils.utils import get_chatbot_response
import json
import pandas as pd
import dotenv
dotenv.load_dotenv()
...
class RecommendationAgent():
    ...
    def recommendation_classification(self,messages):
        ...
        Your output should be in a structured json format like so. Each key is a string and each value is a string you must surround each of these with double quotes. Make sure to follow the format exactly:
        {
    ...
    def get_response(self, messages):
        ...
        print(f"-------------------recommendation_classification{type(recommendation_classification)}----------------- \n {json.dumps(recommendation_classification, indent=4)}")
    ...
    def get_apriori_recommendation(self,products,top_k=5):
        ...
        # 1. recommendations : liste finale des produits recommandés (sans doublons, max 2/catégorie)
        # 2. recommendations_per_category : dictionnaire pour compter les recommandations par catégorie
        recommendations = []
        recommendations_per_category = {}
        for recommendation in recommendation_list:
            # 3. Si le produit est déjà recommandé, on passe au suivant (évite les doublons)
            if recommendation['product'] in recommendations:
                continue

            # 4. On récupère la catégorie du produit
            ...
            # 5. Si on a déjà 2 produits de cette catégorie, on saute ce produit
            if recommendations_per_category[product_category] >= 2:
                continue

            # 6. On incrémente le compteur pour cette catégorie
            recommendations_per_category[product_category] += 1

            # 7. On ajoute le produit à la liste des recommandations
            recommendations.append(recommendation['product'])

            # 8. Si on a atteint le nombre maximal de recommandations, on arrête la boucle
